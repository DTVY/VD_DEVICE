 ...
 *
 *  */
/* (20.3.16) config clock again
 * g_uiSamplingFreq cao nhat o day chi duoc toi 25.000, neu len 26.000 thi g_ulBadPeriphIsr2=1
 *
 * */
/*(9.7.16)
 * g_uiSamplingFreq = 12500
 * */
SAMPLING_ADC adcNode[2] =
		{ { .g_uiSamplingFreq = 25000, .g_ucDataReady = 0, .g_ulBadPeriphIsr1 =
				0, .g_ulBadPeriphIsr2 = 0, .g_uluDMAErrCount = 0 } };
//------------------------------------------------------------------------
#define LoadTimer	SysCtlClockGet() / (adcNode[0].g_uiSamplingFreq)
//-------------------------------------------------------------------------
/*
...
void Timer0AIntHandler(void) {
	...
	if (countTimerISRLoopSample > 102400)
...
void Timer2AIntHandler(void) {
	...
	g_uiDSPPerSec = 0;
	ROM_TimerLoadSet(TIMER2_BASE, TIMER_A, SysCtlClockGet());
...
// The amount of data transferred increments in sets of 1024
	...
	if (NUM_SAMPLES > ulDataXferd) {
		...
		TimerLoadSet(TIMER0_BASE, TIMER_A, LoadTimer - 1);
...
// Should go from 0 to 2048
	...
	/*
	 * The function ADCClockConfigSet is required for TM4C129 devices. Also if you look at the function it does not modify the ADCSPC
	 * register. For TM4C123 device you would need to change the ADC Clock Source to PIOSC from default configuration of PLL/25 (when using PLL)
	 * */
	//ADCClockConfigSet();
	...
	UDMA_ATTR_REQMASK);
/*
 * uint32_t g_ulADCValues : kieu du lieu 4 bytes = 4*8= 32 bits
 * UDMA_SIZE_32 & UDMA_DST_INC_32
 * address increment of 32-bit word
 *
 * */
...
void InitSamplingTimer() {
	...
	TimerLoadSet(TIMER0_BASE, TIMER_A, LoadTimer - 1);
	/*
	 * T = 1/Fc = 1/80 Mhz = 12.5 ns
	 * t = T*LoadTimer = 12.5 ns * ((80Mhz / 25000) - 1)===> timer set = 40us
	 * */
    /*(9.7.16)
     * T = 1/Fc = 1/80 Mhz = 12.5 ns
     * t = T*LoadTimer = 12.5 ns * ((80Mhz / 12500) - 1)===> timer set = 80us
     * */
