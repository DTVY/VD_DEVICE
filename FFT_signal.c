...
void InitDSP2(void) {
    /*
     * frequency resolution: 24.41406 Hz
     * */
...
//	arm_rfft_f32(&fftStructure, outFFT, outFFT);
    ...
    arm_cmplx_mag_f32(outFFT, outFFT, NUM_SAMPLES);
    // ignore the DC value
    outFFT[0] = 0.0f;
    uint16_t n = 0;
    ///(50*SAMPLE_RATE)/fftLength;
    ...
    // choose get maxIndex is 256, if over 256, it jump to value 510, reading frequency will be fail
    arm_max_f32(outFFT, NUM_SAMPLES / 2, &fftNode[0].maxValue,
            &fftNode[0].maxIndex);
    /*
     * 17.5.16 :
     * fftNode[0].maxIndex: <12 (<292 Hz) & >256 (>6225 Hz)
     * revent sensitive : (fftNode[0].maxIndex < 8)
     * (9.7.16)  delta F = Fsampling/N = 12.2Hz
     * (IF signal) 3kHz, 3kHz/12.2 = 245 (256 real value FFT)
     *  ==> 512 FFT points
     * */
    if ((fftNode[0].maxIndex < 12) || (fftNode[0].maxIndex >= 256)) // xem test len toi 512 lai lan nua
    fftNode[0].maxIndex = 0;
    // calculate frequency value of peak bin
    /*
     * 8/7/16 : final test get true
     * */
    fftNode[0].hertz =
            (fftNode[0].HerztPerBin * (float32_t) fftNode[0].maxIndex);
